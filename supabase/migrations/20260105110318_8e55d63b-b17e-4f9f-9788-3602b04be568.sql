-- Create function to auto-forward webhooks to MT5 (24/7 Background)
CREATE OR REPLACE FUNCTION auto_forward_webhook()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS on_webhook_message_insert ON messages;
CREATE TRIGGER on_webhook_message_insert
  AFTER INSERT ON messages
  FOR EACH ROW
  EXECUTE FUNCTION auto_forward_webhook();